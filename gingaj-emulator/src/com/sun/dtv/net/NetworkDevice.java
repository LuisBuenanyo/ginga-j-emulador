...
public class NetworkDevice implements ScarceResource {
	...
	private static final List<ResourceTypeListener> listeners = Collections.synchronizedList(new LinkedList<ResourceTypeListener>());
	...
	static void removeResourceTypeListener(ResourceTypeListener listener) throws NullPointerException {
	...
	public static NetworkDevice reserveOne(boolean force, long timeoutms,
			ScarceResourceListener listener, int type)
			throws SecurityException, IllegalArgumentException,
            TimeoutException {
		
		if (listener == null || timeoutms < -1) {
			
			throw new IllegalArgumentException();
		}
		
		//Sleep para simular espera
		Random r = new Random();
		try {
		...
		NetworkDevice dev = new NetworkDeviceImpl(listener, type);
		dev.reserve(force, timeoutms, listener);
