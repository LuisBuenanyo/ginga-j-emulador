...
public class DefaultLookAndFeel extends LookAndFeel implements FocusListener
{
	...
	public void bind(Component cmp)
	{
		if (tickWhenFocused && cmp instanceof Label) {
			...
			if (focusListeners.contains(cmp) == false) {
				focusListeners.add(cmp);
			}
		}
	}
	...
	private void drawComponent(Graphics g, Label l, Image icon, Image stateIcon, int preserveSpaceForState) 
	{
		...
		{
			if(l.getTextPosition() == Component.LEFT || l.getTextPosition() == Component.RIGHT)
			{
		...
		{
			if(l.getTextPosition() == Component.TOP || l.getTextPosition() == Component.BOTTOM)
			{
		...
		{
			if(l.getTextPosition() == Component.TOP)
			{
				drawLabelText(g, l, text, x, y, l.getWidth());
				g.drawImage(icon, x, y + font.getHeight() + gap);
			}
			else if(l.getTextPosition() == Component.BOTTOM)
			{
				drawLabelText(g, l, text, x, y + icon.getHeight() + gap, l.getWidth());
				g.drawImage(icon, x, y);
			}
			else if(l.getTextPosition() == Component.RIGHT)
			{
				drawLabelText(g, l, text, x + icon.getWidth() + gap , y, l.getWidth());
				g.drawImage(icon, x, y);
			}
			else if(l.getTextPosition() == Component.LEFT)
			{
				drawLabelText(g, l, text, x, y, l.getWidth());
	...
	private int drawLabelString(Graphics g, Label l, String text, int x, int y, int textSpaceW, int preserveSpaceForState) {
		...
		if (l.getTextPosition() == Component.RIGHT) {
			txtX = (l.getWidth() - style.getPadding(Component.LEFT) - textSpaceW);
		} else {
			...
			final int cellOffsetY, final Dimension cellsPreferredSize, final Dimension contentPaneSize)
	{
		...
		cell.setIcon(icon);
		updateCellLook(tp, cell, isSelected);
		if (isSelected) 
		{
	...
	private int getAvaliableSpaceForText(Label l) {
		Style style = l.getStyle();
		int textSpaceW = l.getWidth() - style.getPadding(Component.RIGHT) - style.getPadding(Component.LEFT);
		...
		if (icon != null && (l.getTextPosition() == Component.RIGHT || l.getTextPosition() == Component.LEFT)) {
